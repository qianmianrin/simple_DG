...
__global__ void apply_ghost_cells_kernel(Element *d_Mesh, Element *d_Ghost_bottom, Element *d_Ghost_top, 
    ...
    if (id < Ny) {
        ...
        if (fabs(y_center) <= 0.05) {
            // 保持原样：在射流口注入固定状态 (Dirichlet Inflow)
            // 假设 N_MODE > 0，这里只对平均值 U[v][0] 赋值，高阶模态设为 0 以保持常数输入
            for(int v=0; v<NUM_VARS; v++) {
                for(int k=1; k<N_MODE; k++) d_Ghost_left[id].U[v][k] = 0.0;
            }
            d_Ghost_left[id].U[0][0] = rho1; 
            d_Ghost_left[id].U[1][0] = rho1 * u1;
            d_Ghost_left[id].U[2][0] = rho1 * v1; 
            d_Ghost_left[id].U[3][0] = E1;
        } else {
            // 修改点：在 |y| > 0.05 区域，从计算域内部最左列（index 0）复制数据实现出流
            for(int v=0; v<NUM_VARS; v++) for(int k=0; k<N_MODE; k++) {
                d_Ghost_left[id].U[v][k] = d_Mesh[id * Nx + 0].U[v][k];
            }
        }
        // -----------------------
...
}// 辅助函数：严格基于基础物理定义计算单元中心波速，无任何虚假截断
__device__ static inline void get_local_wave_speed(Element *cell, double &beta_x, double &beta_y) {
    double rho = cell->U[0][0];
    double u = cell->U[1][0] / rho;
    double v_vel = cell->U[2][0] / rho;
    
    // 严格依赖状态方程，不使用人为的 safe_sqrt 和对负压硬返回的掩盖逻辑
    double p = (gamma_gas - 1.0) * (cell->U[3][0] - 0.5 * rho * (u * u + v_vel * v_vel));
    ...
    double max_dev_rho, double max_dev_rhou, double max_dev_rhov, double max_dev_E) 
{
    ...
    // 计算当前及四周边界的波速
    double beta_C_x, beta_C_y; get_local_wave_speed(C_cell, beta_C_x, beta_C_y);
    double beta_L_x, beta_L_y; get_local_wave_speed(L_cell, beta_L_x, beta_L_y);
    double beta_R_x, beta_R_y; get_local_wave_speed(R_cell, beta_R_x, beta_R_y);
    double beta_B_x, beta_B_y; get_local_wave_speed(B_cell, beta_B_x, beta_B_y);
    double beta_T_x, beta_T_y; get_local_wave_speed(T_cell, beta_T_x, beta_T_y);

    // 确立界面最大波速，将特征尺度向高能侧(射流侧)倾斜
    double beta_face_L = fmax(beta_C_x, beta_L_x);
    double beta_face_R = fmax(beta_C_x, beta_R_x);
    double beta_face_B = fmax(beta_C_y, beta_B_y);
    double beta_face_T = fmax(beta_C_y, beta_T_y);
    ...
    for (int m = 0; m <= 2; m++) {
        ...
        double coef = (2.0 * m + 1.0) / ( (m == 1 ? 6.0 : 12.0));        
        for (int v_idx = 0; v_idx < NUM_VARS; v_idx++) {
            ...
            for (int d = 0; d < 6; d++) {
                ...
                // 物理信息从交界面导入：独立缩放每个面的跳跃量
                sigma_x += (beta_face_L * jump_L + beta_face_R * jump_R);
                sigma_y += (beta_face_B * jump_B + beta_face_T * jump_T);
            }

            double h_x_pow = m == 1 ? dx : (dx * dx);
            double h_y_pow = m == 1 ? dy : (dy * dy);
            
            double delta_var = (coef * h_x_pow * sigma_x / dx + 
                                coef * h_y_pow * sigma_y / dy) / denom;
...
double theta1 = 1.0;
    if (rho_min < eps) {
        ...
        // 修正点 1：仅对密度 (v = 0) 的高阶模态施加 theta1 限制
        for (int k = 1; k < N_MODE; k++) {
            cell->U[0][k] *= theta1;
        }
        
        // 修正点 2：仅更新测试点上的密度值，动量和能量保持原样
        for (int i = 0; i < 21; i++) {
            U_test[i][0] = Ubar[0] + theta1 * (U_test[i][0] - Ubar[0]);
        }
    }
...
// ==== 修正后的 MaxDevFunctor ====
// 目的：计算物理量在单元内的真实 L_infty 最大偏差，而非仅仅是平均值的偏差
struct MaxDevFunctor {
    Element* mesh;
    double global_avg;
    int var_idx;
    
    MaxDevFunctor(Element* _mesh, double _avg, int _v) 
        : mesh(_mesh), global_avg(_avg), var_idx(_v) {}
        
    __device__ double operator()(const int& idx) const {
        double max_dev = 0.0;
        Element* cell = &mesh[idx];

        // 1. 检查 9 个体积分点 (Volume Quadrature Points)
        for (int q = 0; q < N_QUAD; q++) {
            double val = 0.0;
            for (int k = 0; k < N_MODE; k++) {
                val += cell->U[var_idx][k] * d_phi_vol[k][q];
            }
            double dev = fabs(val - global_avg);
            if (dev > max_dev) max_dev = dev;
        }

        // 2. 检查 12 个面测试点 (Face Quadrature Points)
        // 这一步极为关键，因为 DG 多项式的极值往往出现在单元边界处
        for (int q = 0; q < 3; q++) {
            double val_T = 0.0, val_B = 0.0, val_L = 0.0, val_R = 0.0;
            for (int k = 0; k < N_MODE; k++) {
                val_T += cell->U[var_idx][k] * d_phi_face_T[k][q];
                val_B += cell->U[var_idx][k] * d_phi_face_B[k][q];
                val_L += cell->U[var_idx][k] * d_phi_face_L[k][q];
                val_R += cell->U[var_idx][k] * d_phi_face_R[k][q];
            }
            max_dev = fmax(max_dev, fabs(val_T - global_avg));
            max_dev = fmax(max_dev, fabs(val_B - global_avg));
            max_dev = fmax(max_dev, fabs(val_L - global_avg));
            max_dev = fmax(max_dev, fabs(val_R - global_avg));
        }
...
while (current_time < T_END) {
   ...
   for (int stage = 1; stage <= 3; stage++) {
    ...
    // 2. 基于当前状态评估双曲空间通量 L_f(U)
    compute_boundary_faces_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh);
    apply_ghost_cells_kernel<<<gridMaxBoundary, blockSize1D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right);
    compute_rhs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_RHS);

    // 3. 物理推进: 得到未经滤波的新状态 U_h^{n, l+1}
    rk3_physical_stage_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, (stage == 1 ? d_U0 : d_U_tmp), d_RHS, stage, dt);

    // ==========================================
    // 4. 核心修正：必须在物理推进后的【新状态】上评估 OEDG 阻尼
    // ==========================================
    // 更新新状态的边界信息与单元顶点导数
    compute_boundary_faces_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh);
    apply_ghost_cells_kernel<<<gridMaxBoundary, blockSize1D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right);
    ...
    // 提取新状态的全局最大偏差 (保留你之前修正的 MaxDevFunctor)
    for(int v = 0; v < NUM_VARS; v++) {
