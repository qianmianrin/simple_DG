...
static inline double max_speed() { 
    ...
    for (int i = 0; i < Nx * Ny; i++) {
        ...
        max_v =fmax( fmax(max_v, fabs(u) + c),800+c_jet);
...
void apply_ghost_cells(void) {
    ...
    const double rho1 = 5.0;
    const double u1   = 800;
    const double v1   = 0;
    const double p1   = 0.4127;
    ...
    for (int i = 0; i < Nx; i++) {
        for (int var = 0; var < NUM_VARS; var++) {
        for (int p = 0; p < 3; p++) {
    Ghost_bottom[i].face_top[var][p] = Mesh[i].face_bottom[var][p];
        }
    }
    }
 ...
 for (int i = 0; i < Nx; i++) {
        for (int var = 0; var < NUM_VARS; var++) {
        for (int p = 0; p < 3; p++) {
    Ghost_top[i].face_bottom[var][p] = Mesh[i  + (Ny - 1)*Nx].face_top[var][p];
        }
    }
    }
    // 右边界 
    ...
    for (int j = 0; j < Ny; j++) {
        for (int p = 0; p < 3; p++) {
            if((yL+dy*(0.5+j))<=0.05 &&(yL+dy*(0.5+j)) >=-0.05){
            ...
            Ghost_left[j].face_right[3][p] = E1;}
                    ...
                    Ghost_left[j].face_right[0][p] = rho0;
            Ghost_left[j].face_right[1][p] = rho0 * u0;
            Ghost_left[j].face_right[2][p] = rho0 * v0;
            Ghost_left[j].face_right[3][p] = E0;
            }
        }
    }
...
void apply_limiter(void) {
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            const double rho1 = 5.0, u1 = 800, v1 = 0, p1 = 0.4127;
    const double E1 = p1 / (gamma - 1.0) + 0.5 * rho1 * (u1 * u1 + v1 * v1);
    
    const double rho0 = 5.0, u0 = 0.0, v0 = 0.0, p0 = 0.4127;
    const double E0 = p0 / (gamma - 1.0);
    ...
    for(int v=0; v<NUM_VARS; v++) {
              ...
              // --- 左边界 (x=0): 始终为波后状态 ---
                if (ii > 0) UbarL[v] = Mesh[jj*Nx + ii-1].U[v][0];
                    ...
                    if (v==0) UbarL[v] = rho0;
                    else if (v==1) UbarL[v] = rho0 * u0;
                    else if (v==2) UbarL[v] = rho0 * v0;
                    else UbarL[v] = E0;
                }
...
void output_results(double t) {
    FILE *fp = fopen("testresult.dat", "w");
    if (fp == NULL) return;
