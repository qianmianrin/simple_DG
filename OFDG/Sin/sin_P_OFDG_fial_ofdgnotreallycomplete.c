    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            for (int k = 0; k < N_MODE; k++) {
                ...
                for (int v = 0; v < NUM_VARS; v++) {
                    cell->U[v][k] = sum_Qphi[v] * M_diag_inv[k] ; 
                }
            }
        }
    }
...
void apply_ghost_cells(void) {
    // 1. 将对侧边界单元的完整模态系数 U 复制到 Ghost 单元中 (周期性边界条件)
    for (int i = 0; i < Nx; i++) {
        memcpy(Ghost_bottom[i].U, Mesh[(Ny - 1) * Nx + i].U, NUM_VARS * N_MODE * sizeof(double));
        memcpy(Ghost_top[i].U, Mesh[0 * Nx + i].U, NUM_VARS * N_MODE * sizeof(double));
    }
    for (int j = 0; j < Ny; j++) {
        memcpy(Ghost_left[j].U, Mesh[j * Nx + (Nx - 1)].U, NUM_VARS * N_MODE * sizeof(double));
        memcpy(Ghost_right[j].U, Mesh[j * Nx + 0].U, NUM_VARS * N_MODE * sizeof(double));
    }

    // 2. 重新计算 Ghost 单元在各个面上的高斯点物理值（Face Values）
    for (int i = 0; i < Nx; i++) {
 ...
 * ============================================================ */void rk3_step(double dt) {
    ...
    // =================================================================
    // Stage 1
    compute_rhs(RHS_buf); // 计算 RHS_buf(U^n)
    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            get_element_damp_coefficients(ii, jj, damp);
            damp[0] = 0.0; // 零阶模态物理守恒
            double a0 = fmax(damp[1], damp[2]); // 计算该单元的局部 a_0 常数

            for (int v = 0; v < NUM_VARS; v++) {
                // k = 0 (无阻尼)
                U1[idx][v][0] = U0[idx][v][0] + dt * RHS_buf[idx][v][0];

                // k > 0 (高阶模态)
                for (int k = 1; k < N_MODE; k++) {
                    double sigma = damp[order_map[k]];
                    double term_inner = U0[idx][v][k] + dt * RHS_buf[idx][v][k] - dt * sigma * U0[idx][v][k] + dt * a0 * U0[idx][v][k];
                    U1[idx][v][k] = exp(-a0 * dt) * term_inner;
                }
            }
        }
    }
    ...
    // =================================================================
    // Stage 2
    compute_rhs(RHS_buf); // 计算 RHS_buf(U^(1))
    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            get_element_damp_coefficients(ii, jj, damp);
            damp[0] = 0.0;
            double a0 = fmax(damp[1], damp[2]);

            for (int v = 0; v < NUM_VARS; v++) {
                // k = 0
                U2[idx][v][0] = 0.75 * U0[idx][v][0] + 0.25 * (U1[idx][v][0] + dt * RHS_buf[idx][v][0]);

                // k > 0
                for (int k = 1; k < N_MODE; k++) {
                    double sigma = damp[order_map[k]];
                    double term1 = 0.75 * exp(-0.5 * a0 * dt) * U0[idx][v][k];
                    double term_inner = U1[idx][v][k] + dt * RHS_buf[idx][v][k] - dt * sigma * U1[idx][v][k] + dt * a0 * U1[idx][v][k];
                    double term2 = 0.25 * exp(0.5 * a0 * dt) * term_inner;
    ...
    // =================================================================
    // Stage 3
    compute_rhs(RHS_buf); // 计算 RHS_buf(U^(2))
    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            for (int v = 0; v < NUM_VARS; v++) {
                // k = 0
                Mesh[idx].U[v][0] = (1.0 / 3.0) * U0[idx][v][0] + (2.0 / 3.0) * (U2[idx][v][0] + dt * RHS_buf[idx][v][0]);

                // k > 0
                for (int k = 1; k < N_MODE; k++) {
                    double sigma = damp[order_map[k]];
                    double term1 = (1.0 / 3.0) * exp(-a0 * dt) * U0[idx][v][k];
                    double term_inner = U2[idx][v][k] + dt * RHS_buf[idx][v][k] - dt * sigma * U2[idx][v][k] + dt * a0 * U2[idx][v][k];
                    double term2 = (2.0 / 3.0) * exp(-0.5 * a0 * dt) * term_inner;
...
void compute_error(double t_final) {
    double error_L1 = 0.0, error_L2 = 0.0, total_area = 0.0;
    const double rho_0 = 1.0, epsilon = 0.2, u_0 = 1.0, v_0 = 0.5;
    const double Lx = xR - xL, Ly = yR - yL;

    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            int idx = jj * Nx + ii; Element *cell = &Mesh[idx];
            double xc = xL + (ii + 0.5) * dx, yc = yL + (jj + 0.5) * dy;
            ...
            for (int q = 0; q < N_QUAD; q++) {
                ...
                double rho_num = 0.0;
                for (int k = 0; k < N_MODE; k++) rho_num += cell->U[0][k] * phi_vol[k][q];

                double x_shift = fmod(x_phys - u_0 * t_final - xL, Lx);
                if (x_shift < 0) x_shift += Lx; x_shift += xL;
                double y_shift = fmod(y_phys - v_0 * t_final - yL, Ly);
                if (y_shift < 0) y_shift += Ly; y_shift += yL;

                double rho_exact = rho_0 + epsilon * sin(2.0 * M_PI * x_shift / Lx) * sin(2.0 * M_PI * y_shift / Ly);
                double diff = fabs(rho_num - rho_exact), dA = w * (dx * dy / 4.0); 

                error_L1 += diff * dA; error_L2 += diff * diff * dA; total_area += dA;
            }
        }
    }
...
int main(void) {
    ...
    output_results(T_END); compute_error(T_END);
    return 0;
}