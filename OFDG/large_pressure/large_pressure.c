...
#define T_END 0.13
/* ============================================================
 * Zhang-Shu 限制器探测点 (高斯积分节点 + 边界节点)
 * ============================================================ */
#define N_ZS_PTS 5
static const double nodes_ZS[N_ZS_PTS] = {
    -1.0,                 /* 左边界 */
    -0.7745966692414834,  /* 高斯点 1 */
     0.0,                 /* 高斯点 2 */
     0.7745966692414834,  /* 高斯点 3 */
     1.0                  /* 右边界 */
};

double phi_ZS[N_MODE][N_ZS_PTS];
/* ============================================================
 * 数据结构
 * ============================================================ */
typedef struct {
...
void precompute_matrices(void) {
    for (int k = 0; k < N_MODE; k++) {
        ...
        // --- 更新：预计算 5 个 Zhang-Shu 探测点处的基函数值 ---
        for (int pt = 0; pt < N_ZS_PTS; pt++) {
            phi_ZS[k][pt] = legendre_1d(k, nodes_ZS[pt]);
        }
    }
}
...
/* ============================================================
 * Zhang-Shu 保正限制器 (严格保守恒)
 * ============================================================ */
/* ============================================================
 * Zhang-Shu 保正限制器 (精确覆盖所有积分与界面节点)
 * ============================================================ */
    ...
    for (int i = 0; i < Nx; i++) {
        ...
        for (int pt = 0; pt < N_ZS_PTS; pt++) {
            for (int v = 0; v < NUM_VARS; v++) {
                U_test[pt][v] = 0.0;
                for (int k = 0; k < N_MODE; k++) {
                    U_test[pt][v] += cell->U[v][k] * phi_ZS[k][pt];
                }
            }
        }
        ...
        for (int pt = 1; pt < N_ZS_PTS; pt++) {
            if (U_test[pt][0] < rho_min) rho_min = U_test[pt][0];
        }

        double theta1 = 1.0;
        if (rho_min < eps) {
            if (Ubar[0] - rho_min > 1e-14) {
            ...
            // 对高阶模态施加限制
            for (int v = 0; v < NUM_VARS; v++) {
                for (int k = 1; k < N_MODE; k++) cell->U[v][k] *= theta1;
            }
            // 同步更新供压力探测使用
            for (int pt = 0; pt < N_ZS_PTS; pt++) {
                for (int v = 0; v < NUM_VARS; v++) {
                    U_test[pt][v] = Ubar[v] + theta1 * (U_test[pt][v] - Ubar[v]);
                }
            }
        }
        ...
        double theta2 = 1.0;
        for (int pt = 0; pt < N_ZS_PTS; pt++) {
            double p_i = calc_pressure(U_test[pt][0], U_test[pt][1], U_test[pt][2]);
            if (p_i < eps) {
                ...
                for (int iter = 0; iter < 50; iter++) {
                    ...
                    double Ut[NUM_VARS];
                    for (int v = 0; v < NUM_VARS; v++) {
                        Ut[v] = Ubar[v] + t_mid * (U_test[pt][v] - Ubar[v]);
                    }
                    if (calc_pressure(Ut[0], Ut[1], Ut[2]) < eps) {
                        t_R = t_mid;
                    } else {
                        t_L = t_mid;
                    }
        ...
        if (theta2 < 1.0) {
            for (int v = 0; v < NUM_VARS; v++) {
                for (int k = 1; k < N_MODE; k++) cell->U[v][k] *= theta2;
            }
        }
    }
}
