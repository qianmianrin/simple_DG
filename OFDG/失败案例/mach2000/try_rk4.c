...
#define NUM_VARS 4 
#define Mach 800.0 // 可调节为 2000 同样稳定
...
// 修改原有的阻尼施加函数，让它基于当前的 state 状态计算阻尼力
void apply_ofdg_damping(double current_U[Nx * Ny][NUM_VARS][N_MODE], double RHS[Nx * Ny][NUM_VARS][N_MODE]) {
    int order_map[6] = {0, 1, 1, 2, 2, 2}; 
    ...
    for (int idx = 0; idx < Nx * Ny; idx++) {
        ...
        for (int v = 0; v < NUM_VARS; v++) {
            for (int k = 1; k < N_MODE; k++) { 
                // 基于当前阶段的 U 计算阻尼力，并累加到右端项
                RHS[idx][v][k] -= sigma_k[k] * current_U[idx][v][k];
            }
        }
    }
}
...
static double compute_dt(void) { 
    double max_wave = max_speed(); // 你的原函数，求对流特征波速 max(\lambda)
    
    // 提前计算一遍阻尼，为了获取全局最大的 a0
    apply_ghost_cells();
    ...
    double max_a0 = 0.0;
    for (int i = 0; i < Nx * Ny; i++) {
        // 对应论文中的 a0 = \sum \sigma_l
        double local_a0 = (damp_local[i][0] + damp_local[i][1] + damp_local[i][2]);
        if (local_a0 > max_a0) {
            max_a0 = local_a0;
        }
    }
    ...
    // 论文 A239 页的修正 CFL 条件
    return CFL / (max_wave / fmin(dx, dy) + max_a0 / h_K); 
}
 ...
 * ============================================================ */static double K1[Nx * Ny][NUM_VARS][N_MODE];
static double K2[Nx * Ny][NUM_VARS][N_MODE];
static double K3[Nx * Ny][NUM_VARS][N_MODE];
static double K4[Nx * Ny][NUM_VARS][N_MODE];
static double U_tmp[Nx * Ny][NUM_VARS][N_MODE];
...
void rk4_step(double dt) {
    ...
    compute_rhs(K1);
    compute_all_damp_coefficients(); 
    apply_ofdg_damping(U0, K1);
    
    for (int i = 0; i < total; i++) 
        for (int v = 0; v < NUM_VARS; v++) 
            for (int k = 0; k < N_MODE; k++) 
                U_tmp[i][v][k] = U0[i][v][k] + 0.5 * dt * K1[i][v][k];
    
    for (int i = 0; i < total; i++) memcpy(Mesh[i].U, U_tmp[i], NUM_VARS * N_MODE * sizeof(double));
apply_zhang_shu_limiter();
    // -- Stage 2 --
    compute_rhs(K2);
    compute_all_damp_coefficients(); 
    apply_ofdg_damping(U_tmp, K2);
    
    for (int i = 0; i < total; i++) 
        for (int v = 0; v < NUM_VARS; v++) 
            for (int k = 0; k < N_MODE; k++) 
                U_tmp[i][v][k] = U0[i][v][k] + 0.5 * dt * K2[i][v][k];
...
apply_zhang_shu_limiter();
    ...
    compute_rhs(K3);
    compute_all_damp_coefficients(); 
    apply_ofdg_damping(U_tmp, K3);
    
    for (int i = 0; i < total; i++) 
        for (int v = 0; v < NUM_VARS; v++) 
            for (int k = 0; k < N_MODE; k++) 
                U_tmp[i][v][k] = U0[i][v][k] + dt * K3[i][v][k];
...
apply_zhang_shu_limiter();
    ...
    compute_rhs(K4);
    compute_all_damp_coefficients(); 
    apply_ofdg_damping(U_tmp, K4);

    // -- Final Update --
    for (int i = 0; i < total; i++) {
        for (int v = 0; v < NUM_VARS; v++) {
            for (int k = 0; k < N_MODE; k++) {
                Mesh[i].U[v][k] = U0[i][v][k] + (dt / 6.0) * (K1[i][v][k] + 2.0 * K2[i][v][k] + 2.0 * K3[i][v][k] + K4[i][v][k]);
            }
        }
    }
    apply_zhang_shu_limiter();
