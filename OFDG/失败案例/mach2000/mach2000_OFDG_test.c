...
static inline double max_speed() { 
    ...
    for (int i = 0; i < Nx * Ny; i++) {
        ...
        max_v = fmax(fmax(max_v, fabs(u) + c), Mach + c_jet);
...
void apply_ghost_cells(void) {
    ...
    const double rho1 = 5.0, u1 = Mach, v1 = 0.0, p1 = 0.4127;
...
static inline void get_vertex_deriv(int ii, int jj, int vert, double out[6][NUM_VARS]) {
    if (ii >= 0 && ii < Nx && jj >= 0 && jj < Ny) {
        ...
        // v_0=(-1,-1), v_1=(1,-1), v_2=(1,1), v_3=(-1,1)
...
void get_element_damp_coefficients(int ii, int jj, double damp[3]) {
    ...
    get_vertex_deriv(ii, jj, 0, deriv_curr[0]); // v_0
    get_vertex_deriv(ii, jj, 1, deriv_curr[1]); // v_1
    get_vertex_deriv(ii, jj, 2, deriv_curr[2]); // v_2
    get_vertex_deriv(ii, jj, 3, deriv_curr[3]); // v_3

    get_vertex_deriv(ii, jj - 1, 3, deriv_B[0]); // B_cell 的左上 (v_3)
    get_vertex_deriv(ii, jj - 1, 2, deriv_B[1]); // B_cell 的右上 (v_2)
    
    get_vertex_deriv(ii, jj + 1, 0, deriv_T[0]); // T_cell 的左下 (v_0)
    get_vertex_deriv(ii, jj + 1, 1, deriv_T[1]); // T_cell 的右下 (v_1)
    
    get_vertex_deriv(ii - 1, jj, 1, deriv_L[0]); // L_cell 的右下 (v_1)
    get_vertex_deriv(ii - 1, jj, 2, deriv_L[1]); // L_cell 的右上 (v_2)
    
    get_vertex_deriv(ii + 1, jj, 0, deriv_R[0]); // R_cell 的左下 (v_0)
    get_vertex_deriv(ii + 1, jj, 3, deriv_R[1]); // R_cell 的左上 (v_3)
...
for (int vert = 0; vert < 4; vert++) {
    ...
    for(int v = 0; v < NUM_VARS; v++) {
    U_avg[v] = 0.5 * (U_mean_curr[v] + U_mean_neb[v]);
    }

    double rho_avg, u_avg, v_avg, p_avg;
    get_primitive_vars(U_avg, &rho_avg, &u_avg, &v_avg, &p_avg);

// 构建标准界面特征矩阵 (此时内部计算绝对安全，无需 fmax)
if (edge == 0) {
    build_eigen_x(rho_avg, u_avg, v_avg, p_avg, R_dummy, L_mats[vert][edge]);
} else {
    build_eigen_y(rho_avg, u_avg, v_avg, p_avg, R_dummy, L_mats[vert][edge]);
}
...
void compute_all_damp_coefficients(void) {
    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            a0_local[idx] = (damp_local[idx][0] + damp_local[idx][1] + damp_local[idx][2]) / h_K;
        }
    }
}
...
void apply_ofdg_damping(double RHS[Nx * Ny][NUM_VARS][N_MODE]) {
    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            int idx = jj * Nx + ii;
            double *damp = damp_local[idx]; // 【关键修改】直接读取缓存，不调用计算
...
void rk3_step(double dt) {
    ...
    // 【Stage 1】
    compute_rhs(RHS_buf);
    compute_all_damp_coefficients(); // 全局统一计算当前流场的阻尼结构
    
    // 仅在当前时间步的起始阶段确定恒定的指数积分因子 a_0
    for (int i = 0; i < total; i++) {
        a0_local[i] = (damp_local[i][0] + damp_local[i][1] + damp_local[i][2]) / h_K;
    }
    ...
    // 【Stage 2】
    compute_rhs(RHS_buf);
    compute_all_damp_coefficients(); // 依据限制器修正后的中间场 U1 刷新阻尼矩阵 (符合 RK 严格公式)
    apply_ofdg_damping(RHS_buf); 
    ...
    // 【Stage 3】
    compute_rhs(RHS_buf);
    compute_all_damp_coefficients(); // 依据中间场 U2 刷新阻尼矩阵
    apply_ofdg_damping(RHS_buf); 
