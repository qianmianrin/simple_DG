...
__device__ static void build_eigen_x(double rho, double u, double v, double p, double L[NUM_VARS][NUM_VARS]) {
    ...
    L[0][0] = f_ac * (B2 + u * c);   L[0][1] = f_ac * -(B1 * u + c); L[0][2] = f_ac * -(B1 * v); L[0][3] = f_ac * B1;
    double f_en = inv_c2;
    L[1][0] = f_en * (c * c - B2);   L[1][1] = f_en * (B1 * u);      L[1][2] = f_en * (B1 * v);  L[1][3] = f_en * -B1;
    L[2][0] = -v; L[2][1] = 0.0; L[2][2] = 1.0; L[2][3] = 0.0;
    L[3][0] = f_ac * (B2 - u * c);   L[3][1] = f_ac * -(B1 * u - c); L[3][2] = f_ac * -(B1 * v); L[3][3] = f_ac * B1;
}
...
__device__ static void build_eigen_y(double rho, double u, double v, double p, double L[NUM_VARS][NUM_VARS]) {
    ...
    double f_ac = 0.5 * inv_c2;
    L[0][0] = f_ac * (B2 + v * c); L[0][1] = f_ac * -(B1 * u); L[0][2] = f_ac * -(B1 * v + c); L[0][3] = f_ac * B1;
    double f_en = inv_c2;
    L[1][0] = f_en * (c * c - B2); L[1][1] = f_en * (B1 * u);  L[1][2] = f_en * (B1 * v);      L[1][3] = f_en * -B1;
    L[2][0] = u; L[2][1] = -1.0; L[2][2] = 0.0; L[2][3] = 0.0;
    L[3][0] = f_ac * (B2 - v * c); L[3][1] = f_ac * -(B1 * u); L[3][2] = f_ac * -(B1 * v - c); L[3][3] = f_ac * B1;
}
...
__global__ void apply_ofdg_damping_kernel(double d_current_U[Nx*Ny][NUM_VARS][N_MODE], 
                                          double d_RHS[Nx*Ny][NUM_VARS][N_MODE],
                                          double d_damp_local[Nx*Ny][3]) {
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= Nx * Ny) return;
    ...
    double *damp = d_damp_local[id]; 
    double sigma_k[N_MODE];
    sigma_k[0] = 0.0; 
    for (int k = 1; k < N_MODE; k++) {
        int dk = order_map[k];
        double sum_sigma = 0.0;
        for (int l = 0; l <= dk; l++) sum_sigma += damp[l] / h_K;
        sigma_k[k] = sum_sigma;
    }
    
    for (int v = 0; v < NUM_VARS; v++) {
        for (int k = 1; k < N_MODE; k++) { 
            d_RHS[id][v][k] -= sigma_k[k] * d_current_U[id][v][k];
        }
    }
}

__global__ void update_rk_stage_kernel(Element *d_Mesh, double d_U0[Nx*Ny][NUM_VARS][N_MODE], 
                                       double d_K[Nx*Ny][NUM_VARS][N_MODE], 
                                       double d_U_tmp[Nx*Ny][NUM_VARS][N_MODE], double coeff, bool is_final, 
                                       double d_K1[Nx*Ny][NUM_VARS][N_MODE], double d_K2[Nx*Ny][NUM_VARS][N_MODE], 
                                       double d_K3[Nx*Ny][NUM_VARS][N_MODE], double d_K4[Nx*Ny][NUM_VARS][N_MODE], double dt) {
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= Nx * Ny) return;
    ...
    if (!is_final) {
        for (int v = 0; v < NUM_VARS; v++) 
            for (int k = 0; k < N_MODE; k++) {
                d_U_tmp[id][v][k] = d_U0[id][v][k] + coeff * d_K[id][v][k];
                d_Mesh[id].U[v][k] = d_U_tmp[id][v][k];
            }
    } else {
        for (int v = 0; v < NUM_VARS; v++) 
            for (int k = 0; k < N_MODE; k++) 
                d_Mesh[id].U[v][k] = d_U0[id][v][k] + (dt / 6.0) * (d_K1[id][v][k] + 2.0 * d_K2[id][v][k] + 2.0 * d_K3[id][v][k] + d_K4[id][v][k]);
    }
}
...
int main(void) {
    ...
    double (*d_K1)[NUM_VARS][N_MODE], (*d_K2)[NUM_VARS][N_MODE], (*d_K3)[NUM_VARS][N_MODE], (*d_K4)[NUM_VARS][N_MODE];
    cudaMalloc(&d_U0, Nx * Ny * sizeof(*d_U0)); cudaMalloc(&d_U_tmp, Nx * Ny * sizeof(*d_U_tmp));
    cudaMalloc(&d_K1, Nx * Ny * sizeof(*d_K1)); cudaMalloc(&d_K2, Nx * Ny * sizeof(*d_K2));
    cudaMalloc(&d_K3, Nx * Ny * sizeof(*d_K3)); cudaMalloc(&d_K4, Nx * Ny * sizeof(*d_K4));
    ...
    // 【致命 BUG 修复处】避免三元运算符陷阱，正确覆盖完整的边界网格
    ...
    while (t < T_END) {
        ...
        double max_a0   = thrust::transform_reduce(thrust::device, iter, iter + Nx * Ny, DampSumFunctor(d_damp_local), 0.0, thrust::maximum<double>());
        if (max_wave < 1e-9) max_wave = 1.0;
        
        double dt = CFL / (max_wave / fmin(dx, dy) + max_a0 / fmax(dx, dy));
        ...
        // -- Stage 1 --
        compute_rhs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_K1);
        apply_ofdg_damping_kernel<<<gridSize1D, blockSize1D>>>(d_U0, d_K1, d_damp_local);
        update_rk_stage_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_K1, d_U_tmp, 0.5 * dt, false, d_K1, d_K2, d_K3, d_K4, dt);
        ...
        precompute_vertex_derivs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_cell_vertex_derivs);
        compute_damp_coeffs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_cell_vertex_derivs, d_damp_local);
        compute_rhs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_K2);
        apply_ofdg_damping_kernel<<<gridSize1D, blockSize1D>>>(d_U_tmp, d_K2, d_damp_local);
        update_rk_stage_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_K2, d_U_tmp, 0.5 * dt, false, d_K1, d_K2, d_K3, d_K4, dt);
        apply_zhang_shu_limiter_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh);
        ...
        precompute_vertex_derivs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_cell_vertex_derivs);
        compute_damp_coeffs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_cell_vertex_derivs, d_damp_local);
        compute_rhs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_K3);
        apply_ofdg_damping_kernel<<<gridSize1D, blockSize1D>>>(d_U_tmp, d_K3, d_damp_local);
        update_rk_stage_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_K3, d_U_tmp, dt, false, d_K1, d_K2, d_K3, d_K4, dt);
        apply_zhang_shu_limiter_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh);
        ...
        precompute_vertex_derivs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_cell_vertex_derivs);
        compute_damp_coeffs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_cell_vertex_derivs, d_damp_local);
        compute_rhs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_K4);
        apply_ofdg_damping_kernel<<<gridSize1D, blockSize1D>>>(d_U_tmp, d_K4, d_damp_local);
        
        // Final RK4 Update
        update_rk_stage_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_K4, d_U_tmp, 0.0, true, d_K1, d_K2, d_K3, d_K4, dt);
        apply_zhang_shu_limiter_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh);
    ...
    cudaFree(d_U0); cudaFree(d_U_tmp); cudaFree(d_K1); cudaFree(d_K2); cudaFree(d_K3); cudaFree(d_K4);
    return 0;
}