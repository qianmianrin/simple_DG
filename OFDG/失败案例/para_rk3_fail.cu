...
// 定义参数：U_new = c_0 * U_0 + c_prev * (U_prev + dt * RHS)
__global__ void update_ssp_rk3_kernel(Element *d_Mesh, 
                                      double d_U0[Nx*Ny][NUM_VARS][N_MODE], 
                                      double d_U_prev[Nx*Ny][NUM_VARS][N_MODE], 
                                      double d_RHS[Nx*Ny][NUM_VARS][N_MODE], 
                                      double c_0, double c_prev, double dt) {
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= Nx * Ny) return;
    ...
    for (int v = 0; v < NUM_VARS; v++) {
        for (int k = 0; k < N_MODE; k++) {
            // 严格的凸组合更新
            double val = c_0 * d_U0[id][v][k] + c_prev * (d_U_prev[id][v][k] + dt * d_RHS[id][v][k]);
            d_Mesh[id].U[v][k] = val;
        }
    }
}
...
copy_mesh_to_U0<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0);
    ...
    apply_ofdg_damping_kernel<<<gridSize1D, blockSize1D>>>(d_U0, d_RHS, d_damp_local);
    
    // c_0 = 0.0, c_prev = 1.0
    update_ssp_rk3_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_U0, d_RHS, 0.0, 1.0, dt);
    apply_zhang_shu_limiter_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh);

    // ==========================================
    // Stage 2: U^{(2)} = (3/4)*U^{(n)} + (1/4)*(U^{(1)} + dt * RHS(U^{(1)}))
    // ==========================================
    // 将当前的 U^{(1)} 暂存到 d_U_tmp
    ...
    compute_rhs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_RHS);
    apply_ofdg_damping_kernel<<<gridSize1D, blockSize1D>>>(d_U_tmp, d_RHS, d_damp_local);
    
    // c_0 = 0.75, c_prev = 0.25
    update_ssp_rk3_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_U_tmp, d_RHS, 0.75, 0.25, dt);
    apply_zhang_shu_limiter_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh);

    // ==========================================
    // Stage 3: U^{(n+1)} = (1/3)*U^{(n)} + (2/3)*(U^{(2)} + dt * RHS(U^{(2)}))
    // ==========================================
    // 将当前的 U^{(2)} 暂存到 d_U_tmp
    copy_mesh_to_tmp<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U_tmp);
    ...
    // c_0 = 1.0/3.0, c_prev = 2.0/3.0
    update_ssp_rk3_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_U_tmp, d_RHS, 1.0/3.0, 2.0/3.0, dt);
    apply_zhang_shu_limiter_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh);
