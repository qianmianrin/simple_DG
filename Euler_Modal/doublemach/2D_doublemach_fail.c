...
void apply_ghost_cells(void) {
    ...
    // 1. 下边界 (Bottom, y = 0)
    for (int i = 0; i < Nx; i++) {
        double xc = xL + (i + 0.5) * dx; // 单元中心 x
        for (int p = 0; p < 3; p++) {
            // 计算当前面上 Gauss 点的精确 x 坐标
            double xp = xc + 0.5 * dx * nodes1D[p];
            
            if (xp < 1.0 / 6.0) {
                // 激波后流入
                Ghost_bottom[i].face_top[0][p] = rho1;
                Ghost_bottom[i].face_top[1][p] = rho1 * u1;
                Ghost_bottom[i].face_top[2][p] = rho1 * v1;
                Ghost_bottom[i].face_top[3][p] = E1;
            } else {
                // 反射滑移墙边界: 密度、x动量、能量镜像，y动量取反
                Ghost_bottom[i].face_top[0][p] = Mesh[0 * Nx + i].face_bottom[0][p];
                Ghost_bottom[i].face_top[1][p] = Mesh[0 * Nx + i].face_bottom[1][p];
                Ghost_bottom[i].face_top[2][p] = -Mesh[0 * Nx + i].face_bottom[2][p];
                Ghost_bottom[i].face_top[3][p] = Mesh[0 * Nx + i].face_bottom[3][p];
            }
        }
    }
    ...
    for (int i = 0; i < Nx; i++) {
        ...
        for (int p = 0; p < 3; p++) {
            double xp = xc + 0.5 * dx * nodes1D[p];
            if (xp < xs_top) {
                Ghost_top[i].face_bottom[0][p] = rho1;
                Ghost_top[i].face_bottom[1][p] = rho1 * u1;
                Ghost_top[i].face_bottom[2][p] = rho1 * v1;
                Ghost_top[i].face_bottom[3][p] = E1;
            } else {
                Ghost_top[i].face_bottom[0][p] = rho0;
                Ghost_top[i].face_bottom[1][p] = rho0 * u0;
                Ghost_top[i].face_bottom[2][p] = rho0 * v0;
                Ghost_top[i].face_bottom[3][p] = E0;
            }
        }
    }

    // 3. 左边界 (Left, x = 0.0): 始终为激波后状态
    for (int j = 0; j < Ny; j++) {
        for (int p = 0; p < 3; p++) {
            Ghost_left[j].face_right[0][p] = rho1;
            Ghost_left[j].face_right[1][p] = rho1 * u1;
            Ghost_left[j].face_right[2][p] = rho1 * v1;
            Ghost_left[j].face_right[3][p] = E1;
        }
    }

    // 4. 右边界 (Right, x = 4.0): 零梯度流出
    for (int j = 0; j < Ny; j++) {
        for (int var = 0; var < NUM_VARS; var++) {
            for (int p = 0; p < 3; p++) {
                Ghost_right[j].face_left[var][p] = Mesh[j * Nx + (Nx - 1)].face_right[var][p];
            }
        }
    }
}
...
void apply_limiter(void) {
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            const double rho1 = 8.0, u1 = 4.125 * sqrt(3.0), v1 = -4.125, p1 = 116.5;
    const double E1 = p1 / (gamma - 1.0) + 0.5 * rho1 * (u1 * u1 + v1 * v1);
    
    const double rho0 = 1.4, u0 = 0.0, v0 = 0.0, p0 = 1.0;
    const double E0 = p0 / (gamma - 1.0);
            // Modal DG 中，第 0 阶模态直接等价于单元均值
    for(int v=0; v<NUM_VARS; v++) {
                // 当前单元的均值（Modal DG 中第 0 阶模态系数）
                Ubar[v] = Mesh[idx].U[v][0];
                
              // --- 左边界 (x=0): 始终为波后状态 ---
                if (ii > 0) UbarL[v] = Mesh[jj*Nx + ii-1].U[v][0];
                else {
                    if (v==0) UbarL[v] = rho1;
                    else if (v==1) UbarL[v] = rho1 * u1;
                    else if (v==2) UbarL[v] = rho1 * v1;
                    else UbarL[v] = E1;
                }

                // --- 右边界 (x=4): 自由流出 (零梯度) ---
                if (ii < Nx-1) UbarR[v] = Mesh[jj*Nx + ii+1].U[v][0];
                else UbarR[v] = Ubar[v];

                // --- 下边界 (y=0): 分段处理 ---
                if (jj > 0) UbarB[v] = Mesh[(jj-1)*Nx + ii].U[v][0];
                else {
                    if (xc < 1.0/6.0) { // 波后流入
                        if (v==0) UbarB[v] = rho1;
                        else if (v==1) UbarB[v] = rho1 * u1;
                        else if (v==2) UbarB[v] = rho1 * v1;
                        else UbarB[v] = E1;
                    } else { // 反射墙 (镜像处理)
                        if (v == 2) UbarB[v] = -Ubar[2]; // y方向动量取反
                        else UbarB[v] = Ubar[v];         // 其他量镜像
                    }
                }

                // --- 上边界 (y=1): 随时间移动的激波 ---
                if (jj < Ny-1) UbarT[v] = Mesh[(jj+1)*Nx + ii].U[v][0];
                else {
                    // 激波在 y=1 处及以上的理论位置
                    double sin60=sqrt(3.0)/2.0;
                  double xs  =  (1.0 / 6.0) + (10.0 * current_time + 0.5) / sin60;
      
                    if (xc < xs) { // 波后
                        if (v==0) UbarT[v] = rho1;
                        else if (v==1) UbarT[v] = rho1 * u1;
                        else if (v==2) UbarT[v] = rho1 * v1;
                        else UbarT[v] = E1;
                    } else { // 波前
                        if (v==0) UbarT[v] = rho0;
                        else if (v==1) UbarT[v] = rho0 * u0;
                        else if (v==2) UbarT[v] = rho0 * v0;
                        else UbarT[v] = E0;
                    }
                }
            }
...
void apply_positivity_limiter(void) {
    ...
    for (int idx = 0; idx < Nx * Ny; idx++) {
        ...
        // 如果均值已经崩溃（理论上在满足CFL时不应发生），必须强制熔断以防程序崩溃
        double p_bar = calc_pressure_safe(Ubar[0], Ubar[1], Ubar[2], Ubar[3]);
        if (Ubar[0] < eps || p_bar < eps) {
            for (int v = 0; v < NUM_VARS; v++) {
                for (int k = 1; k < 9; k++) cell->U[v][k] = 0.0;
            }
            continue; 
        }

        // 收集21个测试点的状态 (9群体积分点 + 12个面积分点)
        double U_test[21][NUM_VARS];
        for(int p=0; p<21; p++) for(int v=0; v<NUM_VARS; v++) U_test[p][v] = 0.0;

        int pt_idx = 0;
        // 1. 体积分点
        for (int q = 0; q < N9; q++) {
            for (int k = 0; k < N9; k++) {
                for (int v = 0; v < NUM_VARS; v++) U_test[pt_idx][v] += cell->U[v][k] * phi_vol[k][q];
            }
            pt_idx++;
        }
        // 2. 四个面的积分点
        for (int q = 0; q < 3; q++) {
            for (int k = 0; k < N9; k++) {
                for (int v = 0; v < NUM_VARS; v++) U_test[pt_idx][v] += cell->U[v][k] * phi_face_T[k][q];
            }
            pt_idx++;
            for (int k = 0; k < N9; k++) {
                for (int v = 0; v < NUM_VARS; v++) U_test[pt_idx][v] += cell->U[v][k] * phi_face_B[k][q];
            }
            pt_idx++;
            for (int k = 0; k < N9; k++) {
                for (int v = 0; v < NUM_VARS; v++) U_test[pt_idx][v] += cell->U[v][k] * phi_face_L[k][q];
            }
            pt_idx++;
            for (int k = 0; k < N9; k++) {
                for (int v = 0; v < NUM_VARS; v++) U_test[pt_idx][v] += cell->U[v][k] * phi_face_R[k][q];
            }
            pt_idx++;
        }
        ...
        if (theta1 < 1.0) {
            for (int v = 0; v < NUM_VARS; v++) {
                for (int k = 1; k < N9; k++) cell->U[v][k] *= theta1;
            }
            // 更新测试点状态
            for (int i = 0; i < 21; i++) {
                for (int v = 0; v < NUM_VARS; v++) {
        ...
        for (int i = 0; i < 21; i++) {
            ...
            if (p_i < eps) {
                // 使用二分法寻找局部缩放因子 t
                double t_L = 0.0, t_R = 1.0, t_mid = 0.5;
                for (int iter = 0; iter < 15; iter++) { // 15次迭代足以满足计算精度
                    double test_U[NUM_VARS];
                    for (int v = 0; v < NUM_VARS; v++) {
                        test_U[v] = Ubar[v] + t_mid * (U_test[i][v] - Ubar[v]);
                    }
                    double p_mid = calc_pressure_safe(test_U[0], test_U[1], test_U[2], test_U[3]);
        ...
        if (theta2 < 1.0) {
            for (int v = 0; v < NUM_VARS; v++) {
                for (int k = 1; k < N9; k++) cell->U[v][k] *= theta2;
            }
        }
    }
}
