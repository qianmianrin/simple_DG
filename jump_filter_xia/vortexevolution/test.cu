...
// ================== 需要修改的宏定义 ==================
#define Nx 1280        // 可根据精度需求调整，例如 100
#define Ny 1280        // 为了保持网格正交比例，建议 y 方向也调整
#define xL 0.0
#define xR 10.0       // 对应区域 [0, 10]
#define yL -5.0
#define yR 5.0        // 对应区域 [-5, 5]
#define gamma_gas 1.4 // 图中指定 \gamma = 1.4

// 移除与原射流问题相关的特征参数
// #define Mach 800.0 (删除此行)

#define T_END 10.0    // 图中指定计算到 T = 10
...
__global__ void compute_damp_coeffs_kernel(Element *d_Mesh, Element *d_Ghost_bottom, Element *d_Ghost_top, 
    ...
    for(int l=1; l<=3; l++) {
        ...
        for(int s=0; s<NUM_VARS; s++) {
            ...
            if (l >= 3) { S_X += 12.0 * pow(dx, 4) * sum_X[3][s]; S_Y += 24.0 * pow(dy, 4) * sum_Y[3][s]; }
            double val = 0.0;
...
struct WaveSpeedFunctor {
    ...
    __device__ double operator()(const int& idx) const {
        ...
        // 移除 Mach 和 c_jet 依赖，返回严谨的局部特征波速
...
void init_condition(void) {
    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            // 2. 遍历单元内的高斯积分点进行 L2 投影
            for (int q = 0; q < N_QUAD; q++) {
                double r = r_quad_h[q];
                double s = s_quad_h[q];
                double w = w_quad_h[q]; // 高斯权重
                ...
                double u = 1.0 - beta * exp(1.0 - r2) * dy_c / (2.0 * M_PI);
                double v = beta * exp(1.0 - r2) * dx_c / (2.0 * M_PI);
                
                double rho_inner = 1.0 - ((gamma_gas - 1.0) / (16.0 * gamma_gas * M_PI * M_PI)) * beta * beta * exp(2.0 * (1.0 - r2));
                double rho = pow(rho_inner, 1.0 / (gamma_gas - 1.0));
                double p = pow(rho, gamma_gas);
    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            for (int q = 0; q < N_QUAD; q++) {
                ...
                // 计算该积分点处的真实精确解
                double dx_c = fabs(x_q - xc_t);
                if (dx_c > 5.0) dx_c = 10.0 - dx_c; 
                double dy_c = fabs(y_q - yc_t);
                if (dy_c > 5.0) dy_c = 10.0 - dy_c;
                double r2 = dx_c * dx_c + dy_c * dy_c;

                double rho_inner = 1.0 - ((gamma_gas - 1.0) / (16.0 * gamma_gas * M_PI * M_PI)) * beta * beta * exp(2.0 * (1.0 - r2));
                double rho_exact = pow(rho_inner, 1.0 / (gamma_gas - 1.0));
