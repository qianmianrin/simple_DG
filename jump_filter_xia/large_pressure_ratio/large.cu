...
// 1. 同步物理面值（严格执行界面处零梯度外推）
__global__ void sync_ghost_faces_kernel_1d(Element *d_Mesh, Element *d_Ghost_left, Element *d_Ghost_right) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        for (int v = 0; v < NUM_VARS; v++) {
            d_Ghost_left->face_right[v] = d_Mesh[0].face_left[v];
            d_Ghost_right->face_left[v] = d_Mesh[Nx - 1].face_right[v];
        }
    }
}

// 2. 同步空间导数（消除边界处的人工导数跳跃引发的耗散）
__global__ void sync_ghost_derivs_kernel_1d(double d_cell_vertex_derivs[Nx][2][N_DERIV][NUM_VARS],
                                            double d_ghost_deriv_L_faceR[N_DERIV][NUM_VARS],
                                            double d_ghost_deriv_R_faceL[N_DERIV][NUM_VARS]) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        for (int d = 0; d < N_DERIV; d++) {
            for (int v = 0; v < NUM_VARS; v++) {
                d_ghost_deriv_L_faceR[d][v] = d_cell_vertex_derivs[0][0][d][v];
                d_ghost_deriv_R_faceL[d][v] = d_cell_vertex_derivs[Nx - 1][1][d][v];
            }
        }
    }
}
__global__ void compute_damp_coeffs_kernel_1d(Element *d_Mesh, double d_cell_vertex_derivs[Nx][2][N_DERIV][NUM_VARS], 
                                              double d_ghost_deriv_L_faceR[N_DERIV][NUM_VARS],
                                              double d_ghost_deriv_R_faceL[N_DERIV][NUM_VARS], double d_damp_local[Nx][N_MODE], double *d_eta) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= Nx) return;
    ...
    for(int d=0; d<N_DERIV; d++) {
        for(int v=0; v<NUM_VARS; v++) {
            double deriv_L_out = (idx == 0) ? d_ghost_deriv_L_faceR[d][v] : d_cell_vertex_derivs[idx - 1][1][d][v];
            double deriv_R_out = (idx == Nx - 1) ? d_ghost_deriv_R_faceL[d][v] : d_cell_vertex_derivs[idx + 1][0][d][v];
            
            double jump_L = fabs(deriv_curr_L[d][v] - deriv_L_out);
            double jump_R = fabs(deriv_curr_R[d][v] - deriv_R_out);
...
__global__ void compute_rhs_kernel_1d(Element *d_Mesh, Element *d_Ghost_left, Element *d_Ghost_right,double d_RHS[Nx][NUM_VARS][N_MODE]) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= Nx) return;
    ...
    for(int v=0; v<NUM_VARS; v++) {
       ...
       U_L_outside[v] = (idx == 0) ? d_Ghost_left->face_right[v] : d_Mesh[idx - 1].face_right[v];
        U_R_outside[v] = (idx == Nx - 1) ? d_Ghost_right->face_left[v] : d_Mesh[idx + 1].face_left[v];
    }
...
// 【新增 1】分配幽灵单元结构及导数存储空间
    Element *d_Ghost_left, *d_Ghost_right;
    cudaMalloc(&d_Ghost_left, sizeof(Element));
    cudaMalloc(&d_Ghost_right, sizeof(Element));

    double (*d_ghost_deriv_L_faceR)[NUM_VARS], (*d_ghost_deriv_R_faceL)[NUM_VARS];
    cudaMalloc(&d_ghost_deriv_L_faceR, N_DERIV * NUM_VARS * sizeof(double));
    cudaMalloc(&d_ghost_deriv_R_faceL, N_DERIV * NUM_VARS * sizeof(double));
...
for (int stage = 1; stage <= 3; stage++) {
            ...
            sync_ghost_faces_kernel_1d<<<1, 1>>>(d_Mesh, d_Ghost_left, d_Ghost_right);

            // ---------------- [阶段 2] 基于解耦架构计算通量并推进时间 ----------------
            compute_rhs_kernel_1d<<<gridSize, blockSize>>>(d_Mesh, d_Ghost_left, d_Ghost_right, d_RHS);
            ...
            // ---------------- [阶段 4] 幽灵界面导数同步 ----------------
            sync_ghost_derivs_kernel_1d<<<1, 1>>>(d_cell_vertex_derivs, d_ghost_deriv_L_faceR, d_ghost_deriv_R_faceL);
            
            // ---------------- [阶段 5] 阻尼评估与限制 ----------------
            compute_damp_coeffs_kernel_1d<<<gridSize, blockSize>>>(
                d_Mesh, d_cell_vertex_derivs, d_ghost_deriv_L_faceR, d_ghost_deriv_R_faceL, d_damp_local, d_eta);
...
cudaFree(d_eta);
    ...
    // 【新增 2】释放幽灵单元显存
    cudaFree(d_Ghost_left);
    cudaFree(d_Ghost_right);
    cudaFree(d_ghost_deriv_L_faceR);
    cudaFree(d_ghost_deriv_R_faceL);
    return 0;
}