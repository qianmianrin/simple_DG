...
// ================== 积分点定义 ==================
// S_G: 3点 Gauss 积分点 (用于通量计算和体积分)
static const double nodes_G_h[3]   = {-0.7745966692414834, 0.0, 0.7745966692414834};
static const double weights_G_h[3] = {0.5555555555555556, 0.8888888888888888, 0.5555555555555556};

// S_GL: 3点 Gauss-Lobatto 积分点 (仅用于 Zhang-Shu 限制器的交叉张量积)
static const double nodes_GL_h[3]   = {-1.0, 0.0, 1.0};
static const double weights_GL_h[3] = {0.3333333333333333, 1.3333333333333333, 0.3333333333333333};
...
double phi_ZS_h[N_MODE][18]; // Zhang-Shu 限制器的 18 个测试点基函数值

__constant__ double d_nodes_G[3];
__constant__ double d_weights_G[3];
...
__constant__ double d_phi_ZS[N_MODE][18];
...
__global__ void compute_rhs_kernel(Element *d_Mesh, Element *d_Ghost_bottom, Element *d_Ghost_top, 
    ...
    // 面上的线积分严格使用 Gauss 积分点 d_weights_G
...
__global__ void apply_zhang_shu_limiter_kernel(Element *d_Mesh) {
    ...
    // 如果物理量的平均值已经为负，这在数学上属于 CFL 违例或理论崩溃，
    // 不作暴力截断（依你规则），保持客观物理状态并退出。
    double p_bar = calc_pressure(Ubar[0], Ubar[1], Ubar[2], Ubar[3]);
    if (Ubar[0] <- eps || p_bar < -eps) return;

    // 严谨提取 18 个测试点上的物理量
    double U_test[18][NUM_VARS];
    ...
    for (int i = 1; i < 18; i++) if (U_test[i][0] < rho_min) rho_min = U_test[i][0];
    ...
    if (rho_min < eps) {
        ...
        for (int i = 0; i < 18; i++) for (int v = 0; v < NUM_VARS; v++) U_test[i][v] = Ubar[v] + theta1 * (U_test[i][v] - Ubar[v]);
    }
...
void init_quadrature_and_matrices(void) {
    // 基础 Galerkin 体积分：严格使用 Gauss x Gauss 张量积，保证积分精度
