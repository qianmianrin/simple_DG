...
// 包含了跳跃滤波器系数 \sigma_j,l 以及混合限制器指示器 \eta_j 的核心计算
__global__ void compute_damp_coeffs_kernel_1d(Element *d_Mesh, double d_cell_vertex_derivs[Nx][2][N_DERIV][NUM_VARS], double d_damp_local[Nx][N_MODE], double *d_eta) {
    ...
    for(int d=0; d<N_DERIV; d++) {
        for(int v=0; v<NUM_VARS; v++) {
            ...
            // 计算混合指示器 \eta_j (基于物理量的边界跳跃求得的无量纲量)
            if (d == 0) {
    ...
    // 降级至 P^2: 只需计算 1 阶和 2 阶多项式的滤波跳跃量系数
    for(int l=1; l<=2; l++) {
        double max_val = 0.0;
...
// 混合跳跃滤波器内核，根据全局 max_eta 动态混合 DG 原始解与滤波解
__global__ void apply_hybrid_jump_filter_kernel_1d(Element *d_Mesh, double d_damp_local[Nx][N_MODE], double *d_eta, double max_eta, double dt) {
    ...
    if (eta_j > 1.0) {
        ...
        for (int v = 0; v < NUM_VARS; v++) {
            for (int k = 1; k < N_MODE; k++) { // k=0是单元平均值，受物理定律保护绝对不参与滤波
...
__global__ void apply_zhang_shu_limiter_kernel_1d(Element *d_Mesh) {
    ...
    // 保密度正定：通过标量系数 theta1 全局缩放避免破坏物理状态
...
void init_condition_lax(void) {
    for (int ii = 0; ii < Nx; ii++) {
        ...
        if (xc < 0.0) { 
            rho = 0.445; u = 0.698; p = 3.528;
        } else {        
            rho = 0.5;   u = 0.0;   p = 0.571;
        }
...
int main(void) {
    ...
    printf("Starting 1D Lax Problem (P2 DG with Hybrid Jump Filter)...\n");
    printf("%-10s  %-14s %-14s\n", "Step", "Time", "dt");
    printf("------------------------------------------\n");

    while (current_time < T_END) {
        // 利用 Thrust 实施准确的全局非负最大波速扫描以确保线性稳定性
        ...
        for (int stage = 1; stage <= 3; stage++) {
            ...
            // 第二重后处理：张-舒 保正限制器以控制密度与压力的严格物理有效性
