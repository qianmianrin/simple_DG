    ...
    for (int jj = 0; jj < Ny; jj++) {
        for (int ii = 0; ii < Nx; ii++) {
            ...
            for (int q = 0; q < N_QUAD; q++) {
                ...
                double x_phys = xc + 0.5 * dx * r;
                double y_phys = yc + 0.5 * dy * s;

                double rho, u, v, p;
                
                double shock_X = 1.0 / 6.0 + y_phys / sqrt(3.0);
                if (x_phys < shock_X) {
                    rho = 8.0; 
                    u = 4.125 * sqrt(3.0); 
                    v = -4.125; 
                ...
                // 【修正点】：绝对不能乘以 0.25，严格遵循高斯积分的原始权值累加
                for (int var = 0; var < NUM_VARS; var++) {
                    for (int k = 0; k < N_MODE; k++) {
                        cell->U[var][k] += w * Q[var] * phi_vol_h[k][q];
                    }
                }
            }

            // 最后统一乘以真实的质量矩阵的逆 (对于 0 阶模态，这里会自动除以 4)
            for (int var = 0; var < NUM_VARS; var++) {
                for (int k = 0; k < N_MODE; k++) {
...
cudaMalloc((void**)&d_UII, Nx * Ny * NUM_VARS * N_MODE * sizeof(double));
    ...
    apply_ghost_cells_kernel<<<gridMaxBoundary, blockSize1D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, 0.0);
    
    // 2. 预计算顶点导数和阻尼系数并应用跳跃滤波
    ...
    // 注意：初始时刻应用跳跃滤波时，dt 应提供一个合理的虚拟时间步长或直接使用初次预估步长
    // 为严格保正，初始时刻最关键的是下方的 Zhang-Shu 限制器
    
    // 3. 应用 Zhang-Shu 限制器，严格保证多项式在所有测试点上不出现负压/负密度
