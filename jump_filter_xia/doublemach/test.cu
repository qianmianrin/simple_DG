...
__global__ void compute_damp_coeffs_kernel(Element *d_Mesh, Element *d_Ghost_bottom, Element *d_Ghost_top, 
                                           Element *d_Ghost_left, Element *d_Ghost_right,
                                           double d_cell_vertex_derivs[Nx*Ny][4][N_DERIV][NUM_VARS],
                                           double d_damp_local[Nx*Ny][4],double *d_eta) {
    int ii = blockIdx.x * blockDim.x + threadIdx.x;
    int jj = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    for(int s = 0; s < NUM_VARS; s++) {
        // 利用0阶导数跳跃量估算论文中的 ||[u_h]|| 
        double current_eta = sum_X[0][s] / (2.0 * dx) + sum_Y[0][s] / (2.0 * dy);
        if (current_eta > cell_eta) cell_eta = current_eta;
    }
    d_eta[idx] = cell_eta;
}
...
__global__ void apply_jump_filter_kernel(Element *d_Mesh, double d_damp_local[Nx*Ny][4], double *d_eta, double max_eta, double dt) {
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= Nx * Ny) return;
    ...
    // 如果单元 eta 超过阈值，且全局存在强间断
    if (eta > 1.0 && max_eta > 1.0) {
        omega = (max_eta - eta) / (max_eta - 1.0);
        omega = fmax(0.0, fmin(1.0, omega)); // 严格限制在 [0, 1] 避免数值越界
    }

    // 当 omega < 1.0 时，说明需要混合低阶滤波多项式
    if (omega < 1.0) {
        for (int k = 1; k < N_MODE; k++) {
            int l = d_mk_map[k] + d_nk_map[k];
            if (l > 0 && l <= 3) {
                double sigma = d_damp_local[id][l];
                // 核心混合： omega * 原始状态 + (1 - omega) * 衰减状态
                double factor = omega + (1.0 - omega) * exp(-sigma * dt);
                for (int v = 0; v < NUM_VARS; v++) {
                    d_Mesh[id].U[v][k] *= factor;
                }
            }
        }
    }
...
int main(void) {
    ...
    double *d_eta;
    cudaMalloc(&d_eta, Nx * Ny * sizeof(double));
    ...
    while (current_time < T_END) {
        ...
        for (int stage = 1; stage <= 3; stage++) {
          ...
          compute_damp_coeffs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_cell_vertex_derivs, d_damp_local, d_eta);
            
          double max_eta = thrust::reduce(thrust::device, d_eta, d_eta + Nx * Ny, 0.0, thrust::maximum<double>());
         apply_jump_filter_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_damp_local, d_eta, max_eta, dt);
    ...
    cudaFree(d_U0); cudaFree(d_U_tmp); cudaFree(d_RHS);cudaFree(d_eta);
    return 0;
}