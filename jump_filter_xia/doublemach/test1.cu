...
int main(void) {
    ...
    while (current_time < T_END) {
        ...
        for (int stage = 1; stage <= 3; stage++) {
            ...
            if (stage == 1) update_rk3_stage_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_U0, d_RHS, stage, dt);//d_Mesh=u1
            else update_rk3_stage_kernel<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_U_tmp, d_RHS, stage, dt);//d_mesh=u2,u3
